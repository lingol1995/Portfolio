...
bool Model::LoadModel(char* filename)
{
	...
	for(int i=0;i<faces.size();i+=3)
	{
		...
		texCoords.push_back (texC[faces[i]-1].getX());
		texCoords.push_back (texC[faces[i]-1].getY());
		normals.push_back (norms[faces[i]-1].getX());
		normals.push_back (norms[faces[i]-1].getY());
		normals.push_back (norms[faces[i]-1].getZ());
	}
