...
TranslationShader::TranslationShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	InitShader(L"../shaders/translate_vs.hlsl", L"../shaders/translate_ps.hlsl");
}


TranslationShader::~TranslationShader()
{
...
void TranslationShader::InitShader(WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	m_device->CreateBuffer(&translateBufferDesc, NULL, &m_translateBuffer);

}


void TranslationShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, float add)
{
	HRESULT result;
...
void TranslationShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, float translate)
{
